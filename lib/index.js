...
var Config = Schema.object({
  ...
  count: Schema.number().step(1).min(LIMITS.minCount).max(LIMITS.maxCount).default(LIMITS.minCount),
...
function apply(ctx, config) {
  ...
  ctx.inject(["attachments"], (imageCtx) => {
    imageCtx.tools.register(defineTool({
      name: "generate_image",
      description: "Generate one or more high-quality images (default: Doubao Seedream 5.0 Pro; can switch to qwen-image-3.0-pro for denser, more accurate text). Calling rules: (1) if the user has not specified the visual style, palette/atmosphere, composition, or the exact wording to place in the image, ask the user first, then call this tool; (2) render in-image text verbatim from the user, never add, drop, or reword characters; (3) always state an explicit art style and palette in the description to avoid the model\u2019s default plain light-color look; (4) default aspect is portrait 2:3 (poster); pass aspect for other ratios; (5) exactly one image is generated by default; pass count (1\u20134) only when the user asked for several candidates or a comparison; (6) the result contains ready-to-use Markdown image links \u2014 embed them verbatim in your reply so the images display inline in the conversation; (7) to modify, restyle, or reference an existing image (image-to-image), pass it via the image parameter (an http(s) URL, an absolute local file path, or a data URI) and describe the desired change in prompt.",
      parameters: {
...
`;
          ...
          const failures = Array.isArray(value.failures) ? value.failures.filter((f) => typeof f === "object" && f !== null && typeof f.message === "string") : [];
          for (const failure of failures) {
            blocks.push({ type: "text", text: `\u26A0 \u7B2C ${failure.index} \u5F20\u751F\u6210\u5931\u8D25\uFF1A${failure.message}` });
          }
          if (inlineBaseUrl !== void 0 && items.length > 0) {
      ...
      async execute(args, exec) {
        ...
        const generateOne = async (index) => {
          ...
          const image = provider === "doubao" ? await doubaoGenerateImage({ apiKey, model, prompt, size, image: imageInput, endpoint, signal }) : await qwenGenerateImage({
            apiKey,
            model,
            prompt,
            size,
