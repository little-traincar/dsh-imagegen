 ...
 *     文案逐字嵌入、显式关水印、落盘 + 会话附件、seed 迭代；默认单张，count 可选（1–4）。
 ...
 * count 未传时返回 undefined，由调用方回落到配置默认值（cfg.count）。
 * @param args - 模型给出的原始入参。
 * @param outDir - 落盘目录。
 * @returns 已校验的 count（可能缺省）与 seed。
 */
function validateRequest(args: Record<string, unknown>, outDir: string): { count?: number; seed?: number } {
  ...
  return { count: count as number | undefined, seed: args.seed as number | undefined }
}
    ...
    imageCtx.tools.register(defineTool({
      ...
      output: {
        ...
        render: (_args, value): ContentBlock[] => {
          ...
          // 旧会话日志回放可能缺字段或元素形状不符：一律降级为空数组，
          // 而不是让 render 抛错（presentation 必须能吞下任意历史数据）。
          ...
          const failures = Array.isArray(value.failures)
            ? value.failures.filter((f): f is { index: number; message: string } =>
                typeof f === 'object' && f !== null && typeof (f as { message?: unknown }).message === 'string')
            : []
          ...
          if (inlineBaseUrl !== undefined && items.length > 0) {
            const links = items.map((item: ImageItemValue) =>
      ...
      async execute(args, exec): Promise<unknown> {
        ...
        // 张数：入参优先，缺省时用配置默认值，并统一夹取到 1–4。
        // 夹取而非抛错：旧会话回放可能带来越界入参，配置也可能被手改成越界值。
